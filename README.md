...
```bash
lambda-hexagonal$ sam logs -n StockCheckerFunction --stack-name lambda-hexagonal --tail
```
...
```bash
lambda-hexagonal$ cd functions/stock-checker
...
```bash
aws cloudformation delete-stack --stack-name lambda-hexagonal
```
